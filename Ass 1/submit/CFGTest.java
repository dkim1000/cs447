...
public class CFGTest {
    ...
    public void addEdge_duplicate() {
        ...
        CFG.Node n = new CFG.Node(1000, m_m, m);
        int sizeOfNodeOne = cfg.edges.get(n).size();

        cfg.addNode(1000, m_m, m);
        cfg.addEdge(1000, m_m, m, 1001, m_m, m);

        assertTrue(cfg.edges.get(n).size() == sizeOfNodeOne);
    }
    ...
    public void deleteNode() {
        ...
        assertFalse(cfg.edges.keySet().contains(nn)); //checking that nn doesn't exist in the edge set

        //check if nn was deleted from every edge value set
        for (Set<CFG.Node> edgePair : cfg.edges.values()) {
            assertFalse(edgePair.contains(nn));
        }
    }
    ...
    //Test checking existing nodes, but unreachable
    @Test
